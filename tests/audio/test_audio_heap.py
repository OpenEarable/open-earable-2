#!/usr/bin/env python3
"""Exercise the linked firmware's real Newlib and LC3 allocation paths.

The application ARM binary runs in isolation, including malloc, _sbrk, and the
LC3 allocator. Only malloc's thread locks are bypassed because the emulator has
no scheduler and every test runs on one host thread. This verifies memory
availability, not Bluetooth timing or microphone hardware.
"""
...
MIN_HEAP_BYTES = 115200
CAPTURE_BYTES = 2048 * 2
...
class Firmware:
    ...
    def __init__(self, path):
        ...
        if self.config.get("CONFIG_AUDIO_DEV") != "1":
            raise AssertionError("This test covers the headset encoder/decoder configuration")

        self.ram_end = (
            int(self.config["CONFIG_SRAM_BASE_ADDRESS"], 0)
    ...
    def allocate(self, size):
        """Hold a real Newlib allocation alongside the codec allocations."""
    ...
    def stop_codec(self):
        # The application retains the shared LC3 tables across recording sessions.
        self.call("sw_codec_lc3_enc_uninit_all")
        self.call("sw_codec_lc3_dec_uninit_all")

    def maximum_waveform_bytes(self):
        return int(self.config["CONFIG_AUDIO_RESPONSE_MAX_SAMPLES"]) * 2
...
class AudioMemoryTests(unittest.TestCase):
    ...
    def test_linked_image_preserves_audio_heap_budget(self):
    ...
    def test_maximum_waveform_and_all_codec_rates_leave_runtime_reserve(self):
        orders = ("waveform-first", "codec-first", "other-memory-first")
        for order, rates in itertools.product(orders, SAMPLE_RATE_PAIRS):
            with self.subTest(order=order, encoder_rate=rates[0], decoder_rate=rates[1]):
                firmware = Firmware(self.elf_path)
                reserve = None
                if order == "waveform-first":
                    waveform = firmware.allocate(firmware.maximum_waveform_bytes())
                elif order == "other-memory-first":
                ...
                firmware.stop_codec()
                firmware.free(waveform)

    def test_measurement_capture_and_recording_resume_fit_with_retained_waveform(self):
        firmware = Firmware(self.elf_path)
        waveform = firmware.allocate(firmware.maximum_waveform_bytes())
        firmware.initialize_codec()
        firmware.start_codec(48000, 48000)

        # A seal measurement stops the LC3 sessions, retains their shared tables,
        # and temporarily allocates its microphone capture.
        firmware.stop_codec()
        capture = firmware.allocate(CAPTURE_BYTES)
        reserve = firmware.allocate(RUNTIME_RESERVE_BYTES)
        firmware.free(reserve)
        firmware.free(capture)

        # The Android app reuses the committed waveform, so recording must be
        # able to resume while the maximum upload remains allocated.
        firmware.start_codec(48000, 48000)
        reserve = firmware.allocate(RUNTIME_RESERVE_BYTES)
        firmware.free(reserve)
        firmware.stop_codec()
    ...
    def test_restarts_reuse_heap_with_retained_waveform(self):
    ...
    def test_insufficient_memory_is_detected(self):
        firmware = Firmware(self.elf_path)
        # Leave less than 48 kHz duplex startup requires. This negative control
        # proves that the emulator does not silently provide unlimited memory.
...
def main():
    ...
    AudioMemoryTests.elf_path = args.elf
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(AudioMemoryTests)
