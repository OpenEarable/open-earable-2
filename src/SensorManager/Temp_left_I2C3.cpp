...
MLX90632 Temp_left_I2C3::temp;

static struct sensor_msg msg_temp_left;

const SampleRateSetting<8> Temp_left_I2C3::sample_rates = {
...
bool Temp_left_I2C3::init(struct k_msgq * queue) {
    ...
    if (!temp.begin(deviceAddress, I2C3, returnError)) {   // hardware I2C mode on I2C3
...
void Temp_left_I2C3::update_sensor(struct k_work *work) {
    ...
    msg_temp_left.data.id = ID_OPTTEMP_left_I2C3;
...
void Temp_left_I2C3::start(int sample_rate_idx) {
    if (!_active) return;
