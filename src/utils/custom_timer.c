...
static const uint32_t TICKS_PER_US = CONFIG_SYS_CLOCK_TICKS_PER_SEC / 1000000;
...
{
    micros_counter += UINT32_MAX; // Increment by the timer interval
}

void custom_timer_start(void)
{
    k_timer_init(&my_timer, timer_expiry_function, NULL);
    k_timer_start(&my_timer, K_USEC(UINT32_MAX), K_USEC(UINT32_MAX)); // Max interval
}
...
uint64_t custom_micros(void)
{
    ...
    uint64_t elapsed_us = elapsed_ticks / TICKS_PER_US;
